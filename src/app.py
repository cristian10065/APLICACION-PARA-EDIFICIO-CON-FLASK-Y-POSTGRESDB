...
def guardar():
    ...
    valor_personas = request.form['personas']
    
    personas = int(valor)/int(valor_personas)
    print (personas)
    
    print(f"datos nombre{nombre} - {cedula} - {valor} - {mes} - {vencimiento} - {tore} - {piso} - {personas}")
    if nombre and cedula and valor and mes and vencimiento and tore and piso and personas:
        if dato == "Agua":
            sql = """
                    INSERT INTO serviceAgua( nombre, cedula, valor, mes, vencimiento, torre, grupo, personas)
                    VALUES ( %s, %s, %s, %s, %s, %s, %s, %s)
                """
            cursor.execute(sql, (nombre, cedula, valor, mes, vencimiento, tore, piso, personas))
            con_bd.commit()
            flash("Registro Guardado Correctamente", "info")
            return render_template('index.html')
        elif dato == "Energia":
            sql = """
                    INSERT INTO serviceluz( nombre, cedula, valor, mes, vencimiento, torre, grupo, personas)
                    VALUES ( %s, %s, %s, %s, %s, %s, %s, %s)
                """
            cursor.execute(sql, (nombre, cedula, valor, mes, vencimiento, tore, piso, personas))
            con_bd.commit()
            ...
            sql = """
                    INSERT INTO servicegas( nombre, cedula, valor, mes, vencimiento, torre, grupo, personas)
                    VALUES ( %s, %s, %s, %s, %s, %s, %s, %s)
                """
            cursor.execute(sql, (nombre, cedula, valor, mes, vencimiento, tore, piso, personas))
            con_bd.commit()
...
def crearTablaAgua():
    ...
    cursor.execute("""
                    CREATE TABLE IF NOT EXISTS serviceAgua( 
                        ...
                        personas character(120)  NOT NULL,
